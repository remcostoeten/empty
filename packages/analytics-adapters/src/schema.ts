...
import { index as pgIndex, pgTable, timestamp, varchar } from "drizzle-orm/pg-core";

export const pageViewsSqlite = sqliteTable(
  "page_views",
  {
  ...
  (table) => ({
    projectCreatedIdx: index("page_views_project_created_idx").on(
      table.projectId,
      table.createdAt
    ),
    projectPathIdx: index("page_views_project_path_idx").on(
      table.projectId,
      table.pathname
    ),
  })
...
export const pageViewsPg = pgTable(
  "page_views",
  {
  ...
  (table) => ({
    projectCreatedIdx: pgIndex("page_views_project_created_idx").on(
      table.projectId,
      table.createdAt
    ),
    projectPathIdx: pgIndex("page_views_project_path_idx").on(
      table.projectId,
      table.pathname
    ),
  })
